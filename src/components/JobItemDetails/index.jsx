...
class JobItemDetails extends Component {
  ...
  getJobItem = async () => {
    ...
    const { id } = this.props.params; // Extract ID from props
    ...
    try {
      ...
      if (response.ok) {
        ...
        const updatedSkillData = data.similar_jobs.map(this.getFormattedSkillData);
      ...
      <div className="full-job-item-container">
        <div className="job-items-container">
          <div className="logo-image-container">
            <img src={companyLogoUrl} alt="job details company logo" className="company-logo-justify" />
          ...
          <ul className="skill-container">
            {skills.map((eachSkill, index) => (
              <SkillsCard key={index} skillDetails={eachSkill} />
            ))}
          </ul>
  ...
  renderFailureView = () => (
    <div className="render-loading-view">
      ...
      <p className="failure-desc">We cannot seem to find the page you are looking for</p>
      <button type="button" className="job-item-failure-button" onClick={this.getJobItem}>
        Retry
      </button>
    </div>
  );
  ...
  render() {
    return (
      <>
        <Header />
        <div className="get-products-details-container">{this.renderJobViews()}</div>
      </>
    );
  }
}
...
function JobItemDetailsWrapper(props) {
